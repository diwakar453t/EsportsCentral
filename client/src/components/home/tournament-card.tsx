...
const TournamentCard = ({ tournament, onRegister }: TournamentCardProps) => {
  ...
  const handleRegister = async () => {
    ...
    try {
      const response = await fetch(`/api/tournaments/${tournament.id}/payment-intent`, {
        method: "POST",
        headers: {
  ...
  return (
    <>
      <Card className="neon-border rounded-xl overflow-hidden bg-dark h-full">
        ...
        <CardContent className="p-6">
          ...
          <div className="flex items-center text-sm text-gray-400 mb-4">
            ...
            <Trophy className="h-4 w-4 ml-4 mr-2 text-yellow-400" /> ${(tournament.prizePool ?? 0).toLocaleString()} Prize Pool
            <Ticket className="h-4 w-4 ml-4 mr-2" /> ${((tournament.entryFee ?? 0) / 100).toFixed(2)} Entry
          </div>
      ...
      {isPaymentDialogOpen && clientSecret && (
        <TournamentPaymentDialog
          tournament={tournament}
